...
//        ekf_filter = new NormalEKF();
        ...
        if (GlobalParam::SOCKET)
        {
            // 创建了socket连接
            // 端口号3000
            udpsender = new UDPSender("192.168.1.13", 3000);
        }
        // 初始上一帧的位置
        last_pose_vec = {0, 0, 0};
    }
...
//        delete ekf_filter;
    ...
    // 两帧识别之间的差值
    Angle_t Predictor::Predict(const cv::Point3d& armor1, const cv::Point3d& armor2, bool is_get_second_armor, int detect_mode, GroundChassisData SerialPortData_, float &frame_delta_t)
    {
        ...
        cv::Point3d tracked_armor_pose; // 跟踪的装甲板
        ...
        if (is_get_second_armor)
        {
            // norm()，计算欧拉距离，L2范数
            // 也就是根据距离判断，判断跟踪的装甲板
            cv::Point3d last_armor(last_pose_vec.x,last_pose_vec.y,last_pose_vec.z);
...
//        predict_point = ekf_filter->predict(shootTime + (shoot_delay / 1000.0f));                         // 预测下一个位置
        ...
        // 预测点的坐标
        cv::Point3d cv_predict_point(predict_point.x(), predict_point.y(), predict_point.z());
    ...
    Angle_t Predictor::ballistic_equation(float gim_pitch, cv::Point3d armor)
    {
