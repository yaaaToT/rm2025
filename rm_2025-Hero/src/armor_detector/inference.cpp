...
void ArmorDetector::drawArmors(cv::Mat &src, std::vector<ArmorObject> &objects)
{
    for(ArmorObject object:objects){
        cv::line(src,object.apex[0],object.apex[1],cv::Scalar(0,255,0),2);  // 红
        cv::line(src,object.apex[1],object.apex[3],cv::Scalar(0,255,0),2);  // 绿
        cv::line(src,object.apex[3],object.apex[2],cv::Scalar(0,255,0),2);  // 蓝
        cv::line(src,object.apex[2],object.apex[0],cv::Scalar(0,255,0),2);  // 白
